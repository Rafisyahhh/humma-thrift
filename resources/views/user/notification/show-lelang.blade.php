                ...
                <div class="col-md-3 border-end">
                    <div class="list-group list-group-flush">
                        @forelse ($notifications as $notification)
                            ...
                            {{-- @if($notification->id === $notificationId) active @endif --}}
                                " style="text-align: left;"
                                href="{{ $notification->data['url'] }}">
                                <h6 class="mb-1 fw-bold
                                {{-- @if($notification->id === $notificationId) text-white @endif --}}
                                " style="font-size: 15px;">{{ $notification->data['title'] }}</h6>
                                <p class="mb-1 @if (!$notification->read_at) fw-bold @endif" style="font-size: 14px;">
                                    {{ Str::limit($notification->data['data'], 100) }}</p>
                                {{-- <small
                    ...
                    <div class="card-body d-flex flex-column flex-lg-row">
                        ...
                        <div class="ms-lg-auto mt-3 d-flex flex-column align-items-end gap-2 mt-lg-0">
                            ...
                            <div class="d-flex gap-2">
                                <form action="{{ route('user.notification.destroy', $notification->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
